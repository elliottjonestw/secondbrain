...
- **Today** — dashboard of the day's schedule (across every visible calendar), due/overdue tasks & reminders, pinned + recent notes, and upcoming birthdays. Schedule events and notes are clickable — they jump straight to that item. **Arrows beside the date step to any other day** (with a "Today" button to come back); the schedule, due items and birthdays follow it, while "overdue" only ever counts against *now*, so it pulls extra items in on today alone. When the assistant is configured, a **written summary of the day** sits above the tiles — see below. Set a **weather location** in Settings and a forecast tile joins the grid, following the day you're looking at.
