...
- **Apple Mail (iCloud), read-only** — connect your iCloud inbox in Settings and the **assistant can read your mail**: search it, and open a message in full. Ask *"did the plumber ever reply?"* or *"what did the school send on Monday?"* and it looks in your actual inbox. It is **read-only by construction** — mailboxes are opened with IMAP's read-only command and bodies are fetched without touching the read flag — so nothing here can send, reply, delete, file, or even mark a message as read, and *not by mistake either*. Nothing is stored: there is no Mail view, no local copy, and no message ever reaches the database. See below.
...
## Apple Mail (IMAP, read-only)

Connect your iCloud inbox and the assistant can read your email. **Read-only, iCloud only, nothing stored.** There is no sending, no Mail view, and no copy of a message anywhere in this app.
...
- **Read-only at the protocol level.** Mailboxes are opened with `EXAMINE` rather than `SELECT`, and bodies are fetched with `BODY.PEEK`. The *server* then refuses anything that would change the mailbox — so read-only is a property of the connection, not a promise about our own code. Even a bug here cannot mark a message read or delete one.
...
Adding another provider (Fastmail, Yahoo) is a preset plus an allowlist entry on both paths; none is shipped. Sending is deliberately out of scope.
...
| `get_message` | one message in full: body text and the list of attachments. **Only present when an inbox is connected** |
