...
- **`CLOUD_SETTING_KEYS` (in `packages/shared`) is a security boundary, not a list.** Settings → Widgets follows the *account* (D1 `space_settings`, migration 0006); everything else stays per-device in `localStorage`. Both sides import that allowlist — the client uploads nothing off it, the route 400s on any key off it — and that is the only thing standing between this sync and the two secrets in `settings.ts` (the OpenAI key, the iCloud app password). Before adding a key, ask whether the value is a secret; if it is, the answer is no. A bad patch is rejected **whole**, so a secret smuggled alongside a legitimate key takes the legitimate key down with it rather than half-landing. Reads stay synchronous — `syncSettingsFromCloud()` runs once at mount and writes into the same localStorage bucket, which doubles as the offline copy; keys edited offline are marked pending and pushed later, never overwritten by the server's older value.
